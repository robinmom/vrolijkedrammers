...
- **Portal-MFA:** Conditional Access op de portal-app is het eerste slot. De API kan als tweede slot `amr = mfa` eisen voor tokens van de portal-app (`Auth__RequirePortalMfa`). **Gecontroleerd op 2026-09-25 met een echt Dev-token:** `environmentAccess` komt mee (`dev,acc`), maar `amr` zit **niet** in de access tokens van External ID. De API-controle blijft daarom uit (`requirePortalMfa = false`); Conditional Access op de portal-app is het MFA-slot.
