...
| Lijst-respons | `{ items[], count }` (**let op:** in de praktijk is `count` het aantal op de pagina, niet het totaal – gezien op 2026-09-26 bij 680 leden; de client pagineert daarom door zolang een pagina vol is); items bevatten minimaal `id` en `memberNumber` (het item-schema verwijst naar `MembersResponseDto`, dat niet volledig in de spec is gedefinieerd) → **detail per lid ophalen** voor de volledige gegevens |
